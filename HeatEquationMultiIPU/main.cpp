...
std::vector<poplar::program::Program> createIpuPrograms(
  ...
  // Create compute sets
  auto compute_set_b_to_a = createComputeSet(graph, b, a, options, "HeatEquation_b_to_a", "HeatEquation");
  auto compute_set_a_to_b = createComputeSet(graph, a, b, options, "HeatEquation_a_to_b", "HeatEquation");
...
int main (int argc, char** argv) {
  try {
    ...
    double inner_volume = (options.height - 2) * (options.width - 2) * (options.depth - 2);
    ...
    std::cout << std::scientific << "wall time per iteration = " << wall_time / options.num_iterations << " s\n";
    ...
    printGeneralInfo(options, flops, bandwidth);
