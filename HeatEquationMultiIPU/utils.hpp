...
void printGeneralInfo(utils::Options &options, double flops, double bandwidth) {
  ...
  std::cout 
    ...
    << "\nParameters"
    << "\n----------"
    << "\nNum. IPUs           = " << options.num_ipus
    << "\nNum. Tiles          = " << options.num_tiles_available
    ...
    << "\nPerformance"
    << "\n-----------"
    << "\nNumber of IPUs & Throughput & Store Bandwidth & Load Bandwidth & Total Bandwidth \\\\" 
    << "\n" << options.num_ipus << " & "
            << tflops << " TFLOPS & " 
            << bandwidth_TB_S << " TB/s & "
            << 7*bandwidth_TB_S << " TB/s & "
            << 8*bandwidth_TB_S << " TB/s \\\\"
    << "\n";
}
